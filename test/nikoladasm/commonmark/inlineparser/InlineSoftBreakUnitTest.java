...
public class InlineSoftBreakUnitTest {
	...
	public void test() {
		...
		assertThat(((InlineTextNode) first).literal(), is(equalTo("foo")));
		Node second = dummyBlockNode.childs().removeFirst();
		assertThat(second, instanceOf(InlineSoftBreakNode.class));
		Node third = dummyBlockNode.childs().removeFirst();
		assertThat(third, instanceOf(InlineTextNode.class));
		assertThat(((InlineTextNode) third).literal(), is(equalTo("baz")));
	}
}
